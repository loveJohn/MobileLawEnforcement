 ...
 * 
 */
public class InspectionEditeView2 extends FrameLayout {
	...
	public InspectionEditeView2(Context context, AttributeSet attrs,
			int defStyle) {
	...
	public InspectionEditeView2(Context context, Activity activity) {
		super(context);
		initView(activity);
	}
	...
	public boolean saveInspection() {
		...
		try {
			...
			DbUtil.insertINSPECTIONEDITDbDatas(getContext(), caseId, keyIllegalSubject, parties, tel, keyIllegalType, keyIllegalStatus, mj, keyLandUsage, notes, user, ddte);
		...
		if (id != null && !id.equals("")) {
			try {
				...
				if (inspectionInfo != null) {
					...
					InspectionEdit inEdit = DbUtil.getInspectionEditByBh(getContext(), caseId);
					if(inEdit!=null){
						illegalSubjectView.setSelectedItem(inEdit.getWfztlb());
