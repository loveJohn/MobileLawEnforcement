...
/**
 */
public class WebCaseAdapter2 extends BaseAdapter {
	...
	int chexiaoButtonVisibility= View.VISIBLE;;
	DecimalFormat kmFormat = new DecimalFormat("#.####");

	public WebCaseAdapter2(Context cxt, ArrayList<ContentValues> its) {
		context = cxt;
		items = its;
	...
	public void setCheXiaoButtonVisibility(int visibility) {
		this.chexiaoButtonVisibility = visibility;
		notifyDataSetChanged();
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		try {
			...
			convertView = inflater.inflate(R.layout.adapter_case2, null);
			...
			holder.chexiaoView = (TextView) convertView
					.findViewById(R.id.adapter_case_chexiao);
			...
			holder.chexiaoView.setTag(convertView);
			...
			holder.chexiaoView.setVisibility(chexiaoButtonVisibility);

			final int p = position;
			holder.chexiaoView.setOnClickListener(new OnClickListener() {
				public void onClick(View arg0) {
					ContentValues itemInfo = items.get(p);
					final String caseId = itemInfo.getAsString(LocalCaseTable.field_id);
					...
					builder.setPositiveButton("确认",
							new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog,
										int which) {
									dialog.dismiss();
//	old								CheXiaoTask task = new CheXiaoTask(
//											context,
