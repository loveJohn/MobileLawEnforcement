...
/**
 * 
* @ClassName MineralLogAdapter 
* @Description TODO
* @author leix@geo-k.cn
* @date 2014年6月12日 上午10:32:57 
*
 */
public class MineralLogAdapter extends BaseAdapter {
	...
	public MineralLogAdapter(Context cxt, ArrayList<ContentValues> its) {
		context = cxt;
		items = its;
...
//		String notes;
		...
		try {
			...
			hasMining = itemInfo.getAsInteger(MilPatrolTable.field_hasMining);
			exception = itemInfo.getAsString(MilPatrolTable.field_exception);
//			notes = itemInfo.getAsString(MilPatrolTable.field_notes);
			
			logTime = itemInfo.getAsString(MilPatrolTable.field_logTime);
...
//			notesView.setText("备注：" + notes);
			ffztmc=itemInfo.getAsString(MilPatrolTable.field_wfztmc);
			szcj=DBHelper.getDbHelper(context).queryAdminFullName(itemInfo.getAsString(MilPatrolTable.field_szcj));
			ffccdbh=itemInfo.getAsString(MilPatrolTable.field_ffckbh);
			ajzt=itemInfo.getAsString(MilPatrolTable.field_ajzt);
		...
		protected Void doInBackground(Integer... params) {
			if (params != null && params.length > 0) {
				try {
					...
					id = itemInfo.getAsString(MilPatrolTable.field_id);
					...
					DBHelper.getDbHelper(context).delete(MilPatrolTable.name,
							MilPatrolTable.field_id + "=?", new String[] { id });// 删除本地案件表数据
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
