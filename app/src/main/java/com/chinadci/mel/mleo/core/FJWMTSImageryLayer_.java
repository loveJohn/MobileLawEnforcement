...
public class FJWMTSImageryLayer_ extends TiledServiceLayer {
	File cacheFolder;
//	static String url = "http://service.fjmap.net/TianDiImage/wmts";
//	static String url2 = "http://service.fjmap.net/cf_img_2014/wmts";
	static String url = "http://service.fjmap.net/cia_fj/wmts";
	static String url2 = "http://t0.tianditu.com/cia_c/wmts";

	public FJWMTSImageryLayer_() {
		super(url);
		initLayer();
	}

	public FJWMTSImageryLayer_(File cacheFolder) {
		super(url);
...
//									new Object[] { url2, "1.0.0",
//											"cf_img_2014", "cf_img_2014",
//											"Matrix_0",
//											String.valueOf(level + 7),
//											String.valueOf(row),
//											String.valueOf(col), "image/tile" });
//				} else {
//					tileUrl = String
//							.format("%1$s?SERVICE=WMTS&REQUEST=GetTile&VERSION=%2$s&LAYER=%3$s&STYLE=%4$s&TILEMATRIXSET=%5$s&TILEMATRIX=%6$s&TILEROW=%7$s&TILECOL=%8$s&FORMAT=%9$s",
//									new Object[] { url, "1.0.0", "TianDiImage",
//											"TianDiImage", "Matrix_0",
//											String.valueOf(level + 7),
//											String.valueOf(row),
//											String.valueOf(col), "image/tile" });
//				}
				if (level + 7 > 7&&level + 7<19) {
					tileUrl = String
							.format("%1$s?SERVICE=WMTS&REQUEST=GetTile&VERSION=%2$s&LAYER=%3$s&STYLE=%4$s&TILEMATRIXSET=%5$s&TILEMATRIX=%6$s&TILEROW=%7$s&TILECOL=%8$s&FORMAT=%9$s",
									new Object[] { url, "1.0.0",
											"cia_fj", "cia_fj",
											"Matrix_0",
											String.valueOf(level + 7),
											String.valueOf(row),
											String.valueOf(col), "image%2Ftile" });
				} else {
					tileUrl = String
							.format("%1$s?SERVICE=WMTS&REQUEST=GetTile&VERSION=%2$s&LAYER=%3$s&STYLE=%4$s&TILEMATRIXSET=%5$s&TILEMATRIX=%6$s&TILEROW=%7$s&TILECOL=%8$s&FORMAT=%9$s",
									new Object[] { url2, "1.0.0", "cia",
											"default", "c",
											String.valueOf(level + 7),
											String.valueOf(row),
											String.valueOf(col), "tiles" });
				}
				Log.i("uri", tileUrl);
				if (cacheFolder == null) {
					...
					String colString = String.format("%1$#010x", col).replace(
							"0x", "C");
					String rowString = String.format("%1$#010x", row).replace(
							"0x", "R");
					String cacheName = new StringBuffer(colString).append(
							".tile").toString();
