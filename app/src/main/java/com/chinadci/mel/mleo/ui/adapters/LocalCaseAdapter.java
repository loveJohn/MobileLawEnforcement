...
/**
 * 
 * @ClassName LocalCaseAdapter
 * @Description TODO
 * @author leix@geo-k.cn
 * @date 2014年6月12日 上午10:32:54
 * 
 */
public class LocalCaseAdapter extends BaseAdapter {
	...
	public LocalCaseAdapter(Context cxt, ArrayList<ContentValues> its) {
		context = cxt;
		items = its;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		try {
			...
			if (valueArea != null && valueArea > 0)
				textArea = kmFormat.format(valueArea) + "m²";
			valueAddress = itemInfo.getAsString(CaseTable.field_address);
			textAddress = valueAddress;
			if (caseTable.equals(LocalCaseTable.name)) {
				try {
					valueAdmin = itemInfo
							.getAsString(LocalCaseTable.field_admin);
			...
			valueIllegalType = itemInfo
					.getAsString(CaseTable.field_illegalType);
			textParties = itemInfo.getAsString(CaseTable.field_parties);
			textCreateTime = TimeFormatFactory2
					.getDisplayTimeM(valueCreateTime);

			ContentValues illegalTypeCV = DBHelper.getDbHelper(context)
					.doQuery(IllegalTypeTable.name, parmeterColumns,
							parmeterSelection,
							new String[] { valueIllegalType });

			textIllegalType = illegalTypeCV
					.getAsString(ParmeterTable.field_name);
			...
			ElemTextView illegalTypeView = new ElemTextView(context);
			illegalTypeView.setText(String.format(
					context.getString(R.string.f_f5), textIllegalType));

			ElemTextView illegalAre = new ElemTextView(context);
			illegalAre.setText(String.format(context.getString(R.string.f_f3),
					textArea));
			...
			holder.keyValuesLayout.addView(illegalTypeView, params);
			holder.keyValuesLayout.addView(illegalAre, params);
		...
		protected Void doInBackground(Integer... params) {
			if (params != null && params.length > 0) {
				try {
					...
					caseId = itemInfo.getAsString(CaseTable.field_id);
