...
public class GetWpzfAjNumTask extends
		AbstractBaseTask<Object, Integer, List<WpzfAjNum>> {
	private Context context;
	private String msg;
//	private SpotsDialog dialog;

	public GetWpzfAjNumTask(Context context,
...
//        dialog.show();
    ...
    protected List<WpzfAjNum> doInBackground(Object... params) {
        ...
        try {
            String param = "user="+params[0]+"&xzqhBm="+params[1]+"&code="+params[2]+"&xfpc="+(params[3]!=null?params[3]:"")+"&aj_year="+(params[4]!=null?params[4]:"")+"&aj_xzqbm="+(params[5]!=null?params[5]:"");		//modify teng.guo
        	String resultStr = HttpUtil.getDataWithoutCookie(appUri, param);
            JSONObject resultObj = new JSONObject(resultStr);
            if (resultObj.optBoolean("succeed")) {
//            	String xzqh = resultObj.optString("xzqhBm");
            	JSONArray objArr = resultObj.optJSONArray("ajList");
                JSONObject obj;
                result = new ArrayList<WpzfAjNum>();
                for (int i = 0; i < objArr.length(); i++) {
                	obj = objArr.optJSONObject(i);
                	WpzfAjNum aj = new WpzfAjNum(obj.optString("KEY"), obj.optString("BL_ZT"), obj.optInt("COUNT"),(String)params[1]);
