...
public class PatrolView3 extends FrameLayout {
	...
	public PatrolView3(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		initView(context);
	}

	public PatrolView3(Context context, AttributeSet attrs) {
		super(context, attrs);
		initView(context);
	}

	public PatrolView3(Context context) {
		super(context);
		initView(context);
	}
				...
				if (annexList != null && annexList.size() > 0) {
					for (ContentValues annex : annexList) {
						...
						if (file.exists()) {
							...
							try {
								String type = MIMEMapTable.getInstance().getParentMIMEType(path.substring(path.lastIndexOf(".")));
